---
title: Robert J. McKeown, PhD, CFA
superuser: true
role: Associate Professor

organizations:
  - name: Department of Economics, York University
    url: "https://profiles.laps.yorku.ca/profiles/mckeow99/"

bio: "Economics educator specializing in quantitative readiness, active learning, and AI-enhanced teaching."

interests:
  - Quantitative-focused Economics Education
  - Generative AI in teaching
  - Financial economics


education:
  courses:
    - course: PhD Economics
      institution: Queen's University
      year: 2017



social:
  - icon: envelope
    icon_pack: fas
    link: "mailto:mckeow99@yorku.ca"
  - icon: graduation-cap
    icon_pack: fas
    link: "https://discover.academics.yorku.ca/Robert.McKeown"
  - icon: linkedin
    icon_pack: fab
    link: "https://www.linkedin.com/in/robert-mckeown-phd-cfa-a2445595/"

email: "mckeow99@yorku.ca"
highlight_name: false
---

Robert J. McKeown, PhD, CFA, is an Associate Professor in the Department of Economics at York University. His work focuses on economics education, quantitative readiness, and technology-enhanced learning in large introductory courses.

He studies how active learning, adaptive platforms, and generative AI can support student success in economics, with recent work on LLM-supported tutoring, ALEKS, and peer-assisted learning. His teaching has been recognized with York University’s Dean’s Award for Excellence in Teaching.

He is the former Academic Director of the Student Numeracy Assistance Centre at Keele (SNACK) and a contributor to *GAMES: A Gentle Approach to Math, Excel, and Stats*, an open educational resource for students preparing for economics, business, and related fields. A recent academic engagement included a visit to the Changchun University of Finance and Economics.

{{< icon name="download" pack="fas" >}} <a href="/uploads/RobertJMcKeown_CV.pdf" target="_blank" rel="noopener">View my Curriculum Vitae</a>